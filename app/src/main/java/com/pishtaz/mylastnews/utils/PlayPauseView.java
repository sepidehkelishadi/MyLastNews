...
public class PlayPauseView extends FrameLayout {
   ...
   // private final PlayPauseDrawable mDrawable;
    ...
    public PlayPauseView(Context context, AttributeSet attrs) {
       ...
       // mBackgroundColor = getResources().getColor(R.color.actionbar_bg);
        mPaint.setAntiAlias(true);
        mPaint.setStyle(Paint.Style.FILL);
      /*  mDrawable = new PlayPauseDrawable(context);
        mDrawable.setCallback(this);*/

        mPauseBackgroundColor = getResources().getColor(android.R.color.transparent);
        mPlayBackgroundColor = getResources().getColor(android.R.color.transparent);
    }

    @Override
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        final int size = Math.min(150, 150);
...
*/
    ...
    protected void onDraw(Canvas canvas) {
        ...
        final float radius = Math.min(mWidth, mHeight) / 12f;
        canvas.drawCircle(mWidth / 12f, mHeight / 12f, radius, mPaint);
